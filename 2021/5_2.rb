...
# Puzzle 10: https://adventofcode.com/2021/day/5#part2
# Felix Wolfsteller
...
def yield_line_points x1,y1,x2,y2
  ...
  elsif horizontal_line?(x1,y1,x2,y2)
    xs = [x1,x2]
    (xs.min..xs.max).each do |x|
      yield Point.new(x,y1)
    end
  else
    point_one = Point.new x1, y1
    point_two = Point.new x2, y2
    smaller_point = (point_one.x < point_two.x) ? point_one : point_two
    larger_point = (point_one.x > point_two.x) ? point_one : point_two
    ((smaller_point.x)..(larger_point.x)).each_with_index do |x, idx|
      y = smaller_point.y + ((smaller_point.y < larger_point.y) ? idx : -idx)
      yield Point.new(x, y)
    end
  end
end

...
Coordinate = Struct.new :x, :y, :z, :w, keyword_init: true do
  def +(vector)
    Coordinate.new x: x + vector[0], y: y + vector[1], z: z + vector[2], w: w + vector[3]
  end
end
...
def puts_map world
  ...
  w_range = Range.new *world.keys.minmax{|a,b| a.w <=> b.w}.map(&:w)
...
ARGF.readlines.each_with_index do |row,row_idx|
  row.chomp.chars.each_with_index do |val,col_idx|
    if val == ACTIVE
      coordinate = Coordinate.new(x: col_idx, y: row_idx, z: 0, w: 0)
...
6.times do |_cycle|
  ...
  w_min_max = *world.keys.minmax{|a,b| a.w <=> b.w}.map(&:w)
  ...
  w_range = Range.new w_min_max.first - 1, w_min_max.last + 1

  dputs "X Range: %s" % x_range.inspect
  dputs "Y Range: %s" % y_range.inspect
  dputs "Z Range: %s" % z_range.inspect
  dputs "W Range: %s" % w_range.inspect

  dputs "After #{_cycle} cycle(s):"
  puts_map world if $DEBUG
